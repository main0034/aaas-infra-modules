...
- **Not yet verified: failure behaviour at `min_replicas = 0`.** A new revision counts as ready once it matches the previous revision's replica count. If the previous revision is scaled to zero, the new one may be declared ready without the init container having run, and a failed migration would then surface on the next request instead of at deploy time. Whether `terraform apply` itself fails on a failed revision is also unestablished. Both need a deliberate update test with a failing migration.
