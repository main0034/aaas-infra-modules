...
The database connection string is generated by Terraform and injected as a Container App secret, surfaced to the container as `DATABASE_URL`. It never appears in git, in plan output, or in Terraform outputs — but it **is** in Terraform state, like every other generated value.

### Why not Key Vault

An earlier version stored the connection string in Key Vault and resolved it via a Key Vault reference. That was removed after the POC, for three reasons:
